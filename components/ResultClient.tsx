...
function SkeletonLoader() {
  return (
    <section className="mx-auto max-w-6xl px-5 pb-14 pt-6 md:pt-10">
      <div className="mb-5 h-16 animate-pulse rounded-3xl bg-slate-100" />
        ...
        <div className="grid gap-6 p-7 md:grid-cols-[1fr_240px] md:p-10">
          <div>
            ...
            <div className="mt-6 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
              {[1, 2, 3, 4].map((i) => <div key={i} className="h-8 animate-pulse rounded-full bg-white/10" />)}
            </div>
          </div>
          <div className="h-48 animate-pulse rounded-[2rem] bg-white/10" />
        </div>
      </div>
...
function ScholarshipCard({ item, index }: { item: Scholarship; index: number }) {
  ...
  return (
    <article className="rounded-[2rem] bg-white p-5 shadow-sm ring-1 ring-slate-100 transition hover:-translate-y-1 hover:shadow-soft">
      ...
      <button type="button" onClick={() => setExpanded(!expanded)} className="mt-3 w-full text-left text-xs font-bold text-brand-600 md:hidden">
        {expanded ? "收起详情 ▲" : "展开详情 ▼"}
      </button>
      <div className={`mt-4 grid gap-2 text-xs font-bold text-slate-700 ${expanded ? "" : "hidden md:grid"}`}>
      ...
      <div className={`mt-4 rounded-2xl bg-green-50 p-4 text-sm leading-6 text-green-950 ring-1 ring-green-100 ${expanded ? "" : "hidden md:block"}`}>
        <strong>匹配理由：</strong>{item.matchReason}
      </div>
      <div className={`mt-3 rounded-2xl bg-rose-50 p-4 text-sm leading-6 text-rose-950 ring-1 ring-rose-100 ${expanded ? "" : "hidden md:block"}`}>
        <strong>风险提示：</strong>{item.risks}
      </div>
...
export function ResultClient() {
      ...
      <section className="mx-auto max-w-2xl px-5 py-14">
        <div className="rounded-[2rem] bg-white p-7 text-center shadow-soft ring-1 ring-slate-100 md:p-10">
          <span className="rounded-full bg-amber-50 px-3 py-1 text-xs font-black text-amber-700 ring-1 ring-amber-100">
            {versionMismatch ? "报告版本已更新" : "缺少测评信息"}
          </span>
          <h1 className="mt-5 text-3xl font-black text-slate-950">
            {versionMismatch ? "请重新生成报告" : "请先完成免费测评"}
          </h1>
          <p className="mt-3 text-sm leading-7 text-slate-600">
            {versionMismatch
              ? "系统已更新，为确保报告信息准确，请重新填写测评表单生成最新报告。"
              : `结果页需要从本机读取你的学历、目标国家、GPA、预算和报告信息。如果你更换了设备或清空了浏览器缓存，请重新测评。`}
          </p>
  ...
  return (
    <section className="mx-auto max-w-6xl px-5 pb-14 pt-6 md:pt-10">
      ...
      {/* Urgency banner */}
      <div className="mt-5 rounded-2xl bg-gradient-to-r from-brand-600 to-brand-700 px-5 py-4 text-sm font-bold text-white">
        本报告基于 {formatDate(matchResult.createdAt)} 的数据生成。奖学金截止日期可能随时更新，建议尽快与顾问确认最新信息。
      </div>

      {/* Profile + Recommended Countries */}
      ...
      <section className="mt-8">
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-3">
          {matchResult.recommendedScholarships.slice(0, FREE_REPORT_LIMIT).map((item, index) => (
            <ScholarshipCard key={item.id} item={item} index={index} />
          ))}
        </div>
      </section>

      {/* Risks + Next Steps + Copy (2-col on mobile instead of 4-col) */}
      <section className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-[2rem] bg-white p-6 shadow-sm ring-1 ring-slate-100">
          ...
          <ul className="mt-4 space-y-3 text-sm leading-6 text-slate-700">{matchResult.risks.map((risk, index) => <li key={risk} className="rounded-2xl bg-slate-50 px-4 py-3"><strong>风险 {index + 1}：</strong>{risk}</li>)}</ul>
        </div>
        <div className="rounded-[2rem] bg-white p-6 shadow-sm ring-1 ring-slate-100">
          <p className="text-sm font-black text-brand-600">Action Plan</p>
          <h2 className="mt-1 text-xl font-black text-slate-950">下一步建议</h2>
          <ul className="mt-4 space-y-3 text-sm leading-6 text-slate-700">{matchResult.nextSteps.map((step, index) => <li key={step} className="rounded-2xl bg-blue-50 px-4 py-3 text-slate-800"><strong>步骤 {index + 1}：</strong>{step}</li>)}</ul>
        </div>
      ...
      {/* Service cards with price anchors */}
      <section className="mt-8">
        <div className="mb-5">
          ...
          {services.map((service) => (
            <div key={service.name} className={`rounded-[2rem] p-6 shadow-sm ring-1 ${service.featured ? "bg-slate-950 text-white ring-slate-800" : "bg-white text-slate-950 ring-slate-100"}`}>
              <div className="flex items-center gap-2">
                ...
                {service.featured ? <span className="inline-flex rounded-full bg-rose-500 px-2.5 py-0.5 text-[10px] font-black text-white">热门</span> : null}
              </div>
              <div className="mt-5 flex items-baseline gap-2">
                <p className="text-4xl font-black">{service.price}</p>
                {service.originalPrice ? <span className="text-lg font-bold text-slate-400 line-through">{service.originalPrice}</span> : null}
              </div>
              ...
              {service.price === "¥99" ? (
                <ServiceActionButton eventName="click_full_report" payload={{ reportId: matchResult.reportId, source: "service_card" }}>获取完整报告</ServiceActionButton>
              ) : service.price === "¥699" ? (
                <ServiceActionButton eventName="click_human_review" payload={{ reportId: matchResult.reportId, source: "service_card" }} variant={service.featured ? "primary" : "secondary"}>预约人工复核</ServiceActionButton>
              ) : (
                <ServiceActionButton eventName="click_add_wechat" payload={{ reportId: matchResult.reportId, source: "free_card" }} variant="secondary">查看当前简版</ServiceActionButton>
              )}
            </div>
          ))}
        </div>
      ...
      {/* Social proof bottom */}
      <div className="mt-6 rounded-2xl bg-slate-50 px-5 py-4 text-center text-sm text-slate-600 ring-1 ring-slate-100">

...
const fieldBaseClass = "mt-2 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3.5 text-sm outline-none transition placeholder:text-slate-400 focus:border-brand-500 focus:ring-4 focus:ring-blue-50";
...
function FieldInput({ field }: { field: { name: string; label: string; placeholder: string; type: string; optional?: boolean } }) {
  return (
    <label className="block">
      ...
      <input name={field.name} type={field.type} required={!field.optional} placeholder={field.placeholder} className={fieldBaseClass} />
...
function FieldSelect({ field }: { field: { name: string; label: string; options: string[] } }) {
  return (
    <label className="block">
      ...
      <select name={field.name} required className={fieldBaseClass}>
    ...
    <div className="mb-8">
      <div className="grid grid-cols-3 gap-2">
        {STEP_LABELS.map((label, index) => (
          <div key={label} className={`rounded-2xl border px-3 py-3 text-center transition ${index === currentStep ? "border-brand-200 bg-blue-50" : index < currentStep ? "border-slate-200 bg-white" : "border-slate-100 bg-slate-50"}`}>
            <div className={`mx-auto flex h-8 w-8 items-center justify-center rounded-full text-xs font-black ${index <= currentStep ? "bg-brand-600 text-white" : "bg-slate-200 text-slate-500"}`}>
              {index < currentStep ? "✓" : index + 1}
            </div>
            <p className={`mt-2 text-xs font-black ${index === currentStep ? "text-brand-700" : "text-slate-500"}`}>{label}</p>
          </div>
        ))}
      </div>
...
export default function AssessmentPage() {
  ...
  function handleNext() {
    ...
    if (!form) return;
    if (validateCurrentStep(form)) setCurrentStep((prev) => Math.min(prev + 1, 2));
  }
  ...
  async function syncLead(userProfile: UserProfile, matchResult: ReturnType<typeof matchScholarships>) {
    try {
      ...
      localStorage.setItem(STORAGE_KEYS.feishuSyncStatus, JSON.stringify({ success: false, status: "failed" as const, error: "后台同步失败", syncedAt: new Date().toISOString() } satisfies FeishuSyncStatus));
    }
  }
  ...
  return (
    <PageShell>
      <section className="mx-auto max-w-4xl px-4 pb-16 pt-8 sm:px-5 md:pt-10">
        {isAnalyzing ? (
          <div className="rounded-[2rem] bg-white p-7 shadow-sm ring-1 ring-slate-200/70 md:p-10">
          ...
          <>
            <div className="mb-6 rounded-[2rem] bg-white p-6 shadow-sm ring-1 ring-slate-200/70 md:p-8">
              ...
              <p className="mt-3 text-sm leading-7 text-slate-600">提交后你会立即看到简版报告；顾问可在飞书工作台查看资料、复核机会并记录跟进状态。</p>
            </div>

            <form id="assessment-form" onSubmit={handleSubmit} className="rounded-[2rem] bg-white p-5 shadow-sm ring-1 ring-slate-200/70 md:p-8">
              <StepProgress currentStep={currentStep} />

              <section className={currentStep !== 0 ? "hidden" : ""}>
              ...
              <section className={currentStep !== 1 ? "hidden" : ""}>
                ...
                <label className="mt-4 block">
                  ...
                  <textarea name="experiences" required rows={5} placeholder="如：一篇 SCI 论文（二作）、一段券商实习、校级数学竞赛一等奖、学生会主席经历等" className={fieldBaseClass} />
                </label>
              </section>
              ...
              {formError ? <p className="mt-5 rounded-2xl bg-rose-50 px-4 py-3 text-sm font-bold text-rose-700 ring-1 ring-rose-100">{formError}</p> : null}
              ...
              <div className="mt-6 flex gap-3">
                ...
                {currentStep < 2 ? (
                  <button type="button" onClick={handleNext} className="flex-1 rounded-2xl bg-brand-600 px-6 py-4 text-sm font-black text-white shadow-sm transition hover:-translate-y-0.5 hover:bg-brand-700">
                    下一步
                  </button>
                ) : (
                  <button type="submit" className="flex-1 rounded-2xl bg-brand-600 px-6 py-4 text-sm font-black text-white shadow-sm transition hover:-translate-y-0.5 hover:bg-brand-700">
                    生成奖学金匹配报告
                  </button>
                )}
              </div>
              <p className="mt-5 text-center text-xs leading-5 text-slate-400">{TRUST_DISCLAIMER}</p>
            </form>
          </>
        )}
      </section>
